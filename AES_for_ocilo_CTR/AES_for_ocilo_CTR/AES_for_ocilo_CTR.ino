...
int pt_number = 1; //1つの鍵当たりのN_pt(平文の数)を設定する変数
...
/* Private functions:                                                        */
/*****************************************************************************/
void CtrBlk(uint8_t *Nonce, uint8_t *CTR, uint8_t *key){ //全128ビットで構成されるカウンタブロックをreturnする
  ...
  if(counter == 0){
    ...
    h = 7 - (counter / 16);//CTRは8bit配列であるので7-する
    w = 15 - (counter % 16);//0~15マデ
    key[h + 8] = w;
    if(w == 0){
      key[h + 7] = 0;
    }
...
void array_print_hex(uint8_t *array_input){//void型であることに注意して使うこと
  int i;
    for (int j = 0; j < 16; j++) {
      sprintf(i, "%02x", array_input[j]);
      Serial.print(i);
    }
}


void Random_Nonce_CounterBlock(unsigned char *pt){
...
uint8_t CTR_Cipher(uint8_t *output, uint8_t *CTR_xor_pt, uint8_t *CTR_output){
  for (int i = 0; i < 16; i++) {
...
void AES128_ECB_encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output)
{
  ...
  BlockCopy(output, input);//outputの内容をinputにコピーしている(この操作いる？ｗ)
  state = (state_t*)output;
  Key = key;//グローバル変数Keyにローカル変数keyをコピーしてる．わざわざkeyをポインタでもらってまで．これいる？ｗって感じなんで要修正
  round10key();//グローバル変数であるRoundKeyをいじる関数.Keyの内容をRoundKeyに書き込んで無理やりRound10の部分鍵にしている．
  //KeyExpansion();
...
void serialEncryption(int pt_number) {
  ...
  uint8_t CTR[8] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  unsigned char test[] = {0x52, 0xe2, 0x30, 0xd0, 0xae, 0xdd, 0x13, 0x7a, 0xbc, 0xea, 0x55, 0xb8, 0x0e, 0xa0, 0xfc, 0xce};
  unsigned char CTR_xor_pt[] = {0x52, 0xe2, 0x30, 0xd0, 0xae, 0xdd, 0x13, 0x7a, 0xbc, 0xea, 0x55, 0xb8, 0x0e, 0xa0, 0xfc, 0xce};//keyの値になる予定の配列
  //test[16] = creat_random_pt(test);//入力の初期値を決めているに過ぎない．下のコードでインクリメントしたりしている．
  //Random_Nonce_CounterBlock(test);
  CtrBlk(Nonce, CTR, CTR_xor_pt);
  ...
  for (int ipt = 0; ipt < N_pt * N_ave; ipt++) {      //ここでloop回数つまり波形の数を決めている　
    for (i = 0; i < 16; i += 1) {       //in[]=test[]つまりtestを平文inに入れている(意味無いので後でtestなくしてinに統合する修正必要)
     in[i] = test[i];
    }
...
/*****************************************************************************/
    delay(1000);
    Serial.println("TraceNumber(DEC): ");
    delay(1000);
...
/*****************************************************************************/
    ...
    CTR_Cipher(output, CTR_xor_pt, CTR_output);//CTR_output[0]にCTR_Chiper[0]のアドレスを代入してる．（注意すること）

    Serial.print("CipherText : ");
    array_print_hex(output);

    Serial.print("\n");

    Serial.print("CTR_xor_pt : ");
    array_print_hex(CTR_xor_pt);
    
    Serial.print("\n");

    Serial.print("CTR Chiptx : ");
    array_print_hex(CTR_output);
