...
void AES128_ECB_encrypt(const uint8_t* input, const uint8_t* key, uint8_t* output)
{
  ...
  digitalWrite(PINNUM, HIGH); //Trigger rise
  // The next function call encrypts the PlainText with the Key using AES algorithm.
  Cipher();
  digitalWrite(PINNUM, LOW);  //Trigger fall
}
...
void serialEvent() {
  ...
  unsigned char test[] = {  0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
  int ave = 1;
  int N_pt = 100;
  int N_ave = 10;

  for (int ipt = 0; ipt < N_pt * N_ave; ipt++) {
    ...
    */
    ...
    //AES-Encryption
    AES128_ECB_encrypt(in, key, output);
    ...
    /*** DEBUG HASHIMOTO ***/
    Serial.print("Key of Last Round : ");
    for (i = 0; i< 16; i++) {
     Serial.print(RoundKey[10*16 + i], HEX);
    }
    Serial.println();
    /***************************************/

    if (ave == N_ave) {
      test[15] = test[15] + 0x01;
      ave = 0;
    }
    ave = ave + 1;
    //delay(50);
